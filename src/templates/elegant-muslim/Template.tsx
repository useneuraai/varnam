...
export default function ElegantMuslimTemplate({ data, isPreview = false }: { data: TemplateData; isPreview?: boolean }) {
              ...
              <div className="w-full mt-4 h-48 border border-[#eed57c]/25 overflow-hidden shadow-inner">
                {isPreview ? (
                  <div className="w-full h-full flex flex-col items-center justify-center bg-neutral-950/80 border border-gold-550/10 text-center p-4">
                    <MapPin className="w-6 h-6 text-gold-300 animate-pulse mb-2" />
                    <span className="text-[10px] font-serif text-gold-200/85 tracking-wide">Venue Map Location</span>
        ...
        try {
          ...
          return sections.map((sec: any, idx: number) => (
            <div key={idx} className="max-w-4xl mx-auto flex flex-col items-center justify-center min-h-screen py-16 px-4 text-center relative z-10 border-t border-emerald-500/10">
              ...
              >
                <span className="font-montserrat text-[10px] tracking-[0.2em] text-[#eed57c] uppercase mb-2">ADDITIONAL DETAILS</span>
                <h2 className="font-cinzel text-xl md:text-2xl text-emerald-900 tracking-widest uppercase mb-8">{sec.title}</h2>
                <div className="w-full bg-[#052e16]/60 border border-[#eed57c]/20 p-6 sm:p-8 rounded-3xl text-left relative overflow-hidden backdrop-blur-sm shadow-2xl">
                  <div className="absolute top-3 right-3 text-[#eed57c]/10 pointer-events-none"><Sparkles className="w-6 h-6" /></div>
                  <p className="font-serif text-sm text-emerald-100/90 leading-relaxed whitespace-pre-line relative z-10">{sec.content}</p>
                </div>
              </motion.div>
            </div>
          ));
