...
import { ArrowRight, MapPin, Calendar, Compass, User, Clock, Sparkles } from "lucide-react";
    ...
    >
      ...
      <div
        ...
        className={`fixed inset-0 bg-cover bg-center bg-no-repeat z-0 pointer-events-none ${
          data.bg_image_url ? "opacity-25" : "opacity-5 mix-blend-overlay"
        }`}
      />
      ...
      {(() => {
        ...
        try {
          ...
          return sections.map((sec: any, idx: number) => (
            <div key={idx} className="max-w-4xl mx-auto flex flex-col items-center justify-center min-h-screen py-16 px-4 text-center relative z-10 border-t border-[#1c1a17]/10">
              <motion.div
              ...
              >
                <span className="font-montserrat text-[9px] tracking-widest text-[#8a857d] uppercase block">ADDITIONAL DETAILS</span>
                <h4 className="font-cinzel text-neutral-800 text-sm font-semibold uppercase tracking-wider my-2">{sec.title}</h4>
                <div className="w-full bg-white/40 border border-[#1c1a17]/10 p-6 sm:p-8 rounded-sm text-left relative overflow-hidden shadow-sm">
                  <div className="absolute top-3 right-3 text-[#1c1a17]/15 pointer-events-none"><Sparkles className="w-5 h-5" /></div>
                  <p className="font-serif text-xs text-[#57534e] leading-relaxed whitespace-pre-line relative z-10">{sec.content}</p>
                </div>
              </motion.div>
            </div>
          ));
